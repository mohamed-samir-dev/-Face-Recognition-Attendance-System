...
import { Calendar, AlertCircle, CheckCircle, XCircle, Coffee, Filter, Trash2 } from "lucide-react";
import { getAllAttendanceHistory, deleteAttendanceRecordsByDate } from "@/lib/services/attendance/adminAttendanceService";
...
export default function AttendanceContent() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteAttendanceRecordsByDate(dateToDelete);
      await loadHistory();
    } catch (error) {
    ...
    <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-white/80 text-xs">{stats.total} Supervisor</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500 hover:bg-red-600 disabled:bg-red-400 text-white rounded-lg transition-colors text-sm"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                        <span>{deletingDate === date ? 'Deleting...' : 'Delete'}</span>
                      </button>
                    </div>
                  </div>
                </div>
      ...
      {showDeleteModal && dateToDelete && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white rounded-lg p-6 max-w-sm w-full mx-4 relative z-10 shadow-xl">
            <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <Trash2 className="w-6 h-6 text-red-600" />
            </div>
            
            <h2 className="text-lg font-bold text-gray-900 mb-2 text-center">Delete Records?</h2>
            
            <p className="text-gray-600 text-sm mb-6 text-center">
              Delete <strong>{groupedHistory[dateToDelete]?.length}</strong> records for <strong>{getDayLabel(dateToDelete)}</strong>?
            </p>
