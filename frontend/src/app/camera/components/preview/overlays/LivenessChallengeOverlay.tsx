...
const challengeConfig: Record<string, { icon: React.ComponentType<{ className?: string }>; title: string; color: string; rotate?: boolean }> = {
...
export default function LivenessChallengeOverlay({ challenge, progress, message }: LivenessChallengeOverlayProps) {
  ...
  return (
    <div className="absolute inset-0 bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center z-10">
      <div className={`bg-linear-to-r ${config.color} text-white px-8 py-4 rounded-2xl shadow-2xl animate-bounce mb-6`}>
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-xl px-6 py-4 shadow-lg max-w-md">
        ...
        <div className="w-full bg-gray-300 rounded-full h-2">
          <div 
            className={`bg-linear-to-r ${config.color} h-full rounded-full transition-all duration-300`}
