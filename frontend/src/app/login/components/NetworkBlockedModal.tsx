...
import { ShieldAlert, Wifi, WifiOff, Globe, X } from "lucide-react";
...
export default function NetworkBlockedModal({ onClose, currentIp }: NetworkBlockedModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] sm:max-w-md mx-auto overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        {/* Top banner */}
        <div className="relative bg-gradient-to-r from-orange-500 to-red-500 px-4 sm:px-6 py-4 sm:py-5 flex flex-col items-center gap-2">
          <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white/20 rounded-full flex items-center justify-center">
            <WifiOff className="w-7 h-7 sm:w-8 sm:h-8 text-white" />
          </div>
          <h2 className="text-white text-base sm:text-lg font-bold text-center">Unauthorized Network</h2>
          <p className="text-orange-100 text-xs font-medium">⚠️ Login Blocked — Unrecognized Network</p>
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-3 bg-gray-50 rounded-xl p-3">
              ...
              <p className="text-xs text-gray-600 leading-relaxed">
                Please connect to the <span className="font-semibold text-gray-800">company WiFi</span> and try again. If you believe this is an error, contact your <span className="font-semibold text-gray-800">System Administrator</span>.
              </p>
            </div>
